  ...
  <div class="container">
    ...
    >
      ...
      <div class="empty" v-if="isLoad && !displayedArticleList.length">
        ...
        <div style="margin-top: 0.5rem">或者手动筛选～</div>
      </div>
      ...
      <van-cell-group>
        ...
        >
          ...
          <template #label>
            <div>
              {{ item.date }}<span style="margin-left: 0.625rem">{{ item.author }}</span>
            </div>
          </template>
        </van-cell>
      </van-cell-group>
      ...
      <van-loading
        style="margin-top: 1.25rem; display: flex; justify-content: center; align-items: center"
        v-if="currentArticleList.length && !scrollDisabled && isLoad"
        >加载中...</van-loading
      >
...
const gotoGithub = () => {
  window.location.href = 'https://github.com/LeekHub/leekhub-news'
}
  ...
  links.forEach((source) => {
    ...
    source.items.forEach((item) => {
      ...
      ranges.forEach((rangeItem) => {
        ...
        if (
          (typeof dates === 'string' && item.date === dates) ||
          (typeof dates !== 'string' && item.date >= dates[0] && item.date <= dates[1])
        ) {
...
const scrollDisabled = computed(() => {
  ...
  return currentLen &&
    currentArticleList.value[currentLen - 1].link ===
      displayedArticleList.value[displayedLen - 1].link
    ? true
    : false
})
...
.search-box {
  ...
  margin: 0 auto;
  padding: 0.625rem;
  box-shadow: 0 0.125rem 0.625rem 0 #f0f0f0;
}
